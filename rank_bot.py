...
#Global variables
curr_season = "division.bro.official.pc-2018-06"
prev_season = "division.bro.official.pc-2018-04"
...
async def checkstats(ctx, Username):
    ...
    initial_r = requests.get(url, headers=header)
    if initial_r.status_code != 200:
        ...
        second_request = requests.get(season_url, headers=header)
        ...
        if len(recent_matches) == 0:
            ...
            if season_losses == 0:
                ...
                await ctx.send(f'ADR = {round(season_damage/games_played,2)}')
                if games_played < 100:
...
async def enlist(ctx,user_IGN):
    ...
    if str(user_id) in server_list:
        ...
        if initial_r.status_code != 200:  
            ...
            #First kick the people with less than 1.9 KD
            if tru_KD < 1.9:
                    await ctx.send(f'Your in-Game KD is: {round(tru_KD,2)}')
                    reason = 'Sorry your KD is less than 1.9'
                    await ctx.send(f'Sorry but you\'ll have to do better to be in here!, {user} will be kicked in 5s')
                    time.sleep(5)
                    await user.send('Sorry but your KD was less than 1.9 and you were kicked!!')
                ...
                if (len(recent_matches) == 0) and (curr_role != 'Dirt'):
                    ...
                    if games_played >= 100:
                        ...
                        if player_KD < 1.9:
                            ...
                            reason = 'Sorry your KD is less than 1.9'
                            member = discord.utils.get(id.members, id=int(curr_user))
                            await member.send('Sorry but your KD was less than 1.9 and you were kicked!!')
...
client.run('NjUxOTAxMjQzMTU3MTE4OTc3.Xegobg.fgZ2w5Ckkwfsi6PEnMhR5j4M51Q')
